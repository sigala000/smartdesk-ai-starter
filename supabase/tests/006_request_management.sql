...
select throws_ok($$select public.transition_request_status('67000000-0000-4000-8000-000000000001','site_visit_proposed','Visit needed',(select updated_at from public.requests where id='67000000-0000-4000-8000-000000000001'))$$,'23514',null,'evidence-dependent site-visit transition fails closed');
select is((select count(*) from public.request_status_history where request_id='67000000-0000-4000-8000-000000000001' and to_status='site_visit_proposed'),0::bigint,'failed prerequisite transition creates no history');
...
select is((select count(*) from public.audit_events where entity_id='67000000-0000-4000-8000-000000000001' and action='request.status_changed'),2::bigint,'request-information status change receives a status audit event');
...
select throws_ok($$select public.transition_request_status('67000000-0000-4000-8000-000000000001','new','Return to intake',(select updated_at from public.requests where id='67000000-0000-4000-8000-000000000001'))$$,'42501',null,'technical officer cannot perform intake transition');
select set_config('request.jwt.claim.sub','62000000-0000-4000-8000-000000000001',true);
select is((select count(*) from public.audit_events where entity_id='67000000-0000-4000-8000-000000000001' and action='request.status_changed'),2::bigint,'forbidden transition creates no audit event');
