...
select is((select count(*) from public.whatsapp_accounts),0::bigint,'production-safe seed has no WhatsApp account');
...
select throws_ok($$insert into public.whatsapp_accounts(organization_id,phone_number_id,whatsapp_business_account_id,is_test) values('10000000-0000-4000-8000-000000000001','22345678901','98765432101',false)$$,'23514',null,'non-test account is rejected in Phase 5b');
...
select is((select count(*) from public.whatsapp_message_deliveries where direction='outbound'),1::bigint,'one reply creates one outbound intent');
...
select ok(public.record_whatsapp_send_result('10000000-0000-4000-8000-000000000001',(select id from public.whatsapp_message_deliveries where direction='outbound'),'sent','wamid.outbound.1',null),'provider success is recorded from sending state');
select isnt(public.record_whatsapp_send_result('10000000-0000-4000-8000-000000000001',(select id from public.whatsapp_message_deliveries where direction='outbound'),'sent','wamid.outbound.2',null),true,'send result cannot be overwritten after completion');
...
select is((select status from public.whatsapp_message_deliveries where direction='outbound'),'read','delivery remains read');
