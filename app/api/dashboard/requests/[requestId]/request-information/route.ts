...
export async function POST(request: Request, context: RouteContext) {
  ...
  const input = requestInformationSchema.safeParse(body.value);
  ...
  if (!runtime.access.ok)
    return apiAccessError(
      runtime.access,
      "You are not authorized to request information.",
    );
  const result = await runtime.service.requestInformation(
    runtime.access.context,
    id.data,
    input.data,
  );
  return result.ok
    ? apiSuccess({ request: result.value }, 201)
