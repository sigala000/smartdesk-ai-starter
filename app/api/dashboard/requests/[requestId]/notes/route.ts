...
import { internalNoteSchema, requestIdSchema } from "@/lib/schemas/request-api";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const input = internalNoteSchema.safeParse(body.value);
  ...
  const result = await runtime.service.addNote(
    runtime.access.context,
    id.data,
    input.data,
  );
  return result.ok
    ? apiSuccess({ note: result.value }, 201)
