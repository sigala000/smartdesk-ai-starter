...
import { digestSecret } from "@/lib/http/public-conversation-cookie";
...
export async function POST(request: Request, context: Context) {
  ...
  const input = confirmRequestSchema.safeParse(body.value);
  if (!input.success) return apiValidation(input.error);
  const access = await publicConversationRequestContext(request, id.data);
  const result = await createPublicConversationRuntime().confirm(
    id.data,
    access.tokenDigest,
    digestSecret(input.data.confirmationNonce),
    input.data,
    access.subjectDigest,
  );
  return result.ok
    ? apiSuccess({ request: result.value }, result.value.replayed ? 200 : 201)
